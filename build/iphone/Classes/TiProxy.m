...
//Rare exceptions to indicate a bug in the _searchandplay code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_searchandplay = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_searchandplay]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.searchandplay.module"
